...
#' for the factors, and a random walk model for the factors. The (innovation) distribution
#' has a several options as well.
#'
...
#' }
#' @param dist_factor prior distribution for the factors (innovations);
#' must be one of
#' \itemize{
...
fdlm = function(Y, tau, K = NULL,
                factor_model = "RW", dist_factor = "NIG",
                nsave = 1000, nburn = 1000, nskip = 3,
                mcmc_params = list("beta", "fk", "mu_k"),
                use_obs_SV = FALSE,
...
fosr = function(Y, tau, X = NULL, K = NULL,
  ...
  if(p > 1)
  {
    ...
    {
      rep_lambda_j = rep(1, (p-1)*K) # Recurring scale term
    }
  ...
  for(nsi in 1:nstot)
  {
    ...
    if(any.missing)
    {
      Y.samp = fdlm_impute(Yna, Btheta, sigma_et = sigma_et, Bmat = splineInfo$Bmat);
      Y = Y.samp$Y;
      BtY = Y.samp$BtY
    }
    #----------------------------------------------------------------------------
    ...
    if(includeBasisInnovation)
    {
      Y_tilde =  tcrossprod(theta, t(Psi));
      sigma_tilde = rep(sigma_nu, T)
    }
    else
    {
    ...
    for(k in 1:K){
      ...
      y_tilde_k = Y_tilde[,k];
      sigma_tilde_k = sqrt(sigma_tilde^2 + sigma_gamma_tk[,k]^2)
      ...
      if(p >= T){
        ...
        chQ_k = chol(
          crossprod(X/sigma_tilde_k) +
            diag(as.numeric(1/c(sigma_mu_k[k],sigma_omega_pk[,k])^2)))
    ...
    sigma_e = 1/sqrt(
      rgamma(n = 1,
             shape = sum(!is.na(Y))/2,
             rate = sum((Y - Btheta)^2, na.rm=TRUE)/2))
    ...
    evolParams_int = sampleEvolParams(
      omega = gamma_tk/rep(sigma_delta_k, each = T), # NOTE:I THINK THIS IS WRONG NOTE:
...
dfosr = function(Y, tau, X = NULL, K = NULL,
  ...
  if(is.na(match(dist_reg_coef, c("DHS", "HS", "NIG", "BL", "SV"))))
    stop("The regression coefficient distribution must be one of 'DHS', 'HS', 'NIG','BL', or 'SV'")
  if(is.na(match(dist_reg_error, c("DHS", "HS", "NIG", "BL", "SV"))))
    stop("The regression error distribution must be one of 'DHS', 'HS', 'NIG', 'BL', or 'SV'")
  #----------------------------------------------------------------------------
...
dfosr_ind = function(Y, tau, X = NULL, K = NULL,
                     ...
                     computeDIC = TRUE){
  #----------------------------------------------------------------------------
  # If number of predictors is moderate, sample all k-factor terms separately
  #----------------------------------------------------------------------------
  if(!is.null(X) && ncol(X) >= 3)
    return(
      dfosr_ind_k(Y = Y, tau = tau, X = X, K = K,
                  use_dynamic_reg = use_dynamic_reg,
                  dist_reg_coef = dist_reg_coef, dist_reg_error = dist_reg_error,
                  nsave = nsave, nburn = nburn, nskip = nskip,
                  mcmc_params = mcmc_params,
...
dfosr_ar = function(Y, tau, X = NULL, K = NULL,
  ...
  if(!is.null(X) && ncol(X) >= 3)
    return(
      dfosr_ar_k(Y = Y, tau = tau, X = X, K = K,
                 use_dynamic_reg = use_dynamic_reg,
                 dist_reg_coef = dist_reg_coef, dist_reg_error = dist_reg_error,
                 nsave = nsave, nburn = nburn, nskip = nskip,
                 mcmc_params = mcmc_params,
...
dfosr_rw = function(Y, tau, X = NULL, K = NULL,
  ...
  if(!is.null(X) && ncol(X) >= 3)
    return(
      dfosr_rw_k(Y = Y, tau = tau, X = X, K = K,
                 use_dynamic_reg = use_dynamic_reg,
                 dist_reg_coef = dist_reg_coef, dist_reg_error = dist_reg_error,
                 nsave = nsave, nburn = nburn, nskip = nskip,
                 mcmc_params = mcmc_params,
...
#' Runs the MCMC for the dynamic function-on-scalars regression model based on
#' an FDLM-type expansion. Here, we assume the factor regression has AR(1) errors.
#' This particular sampler looks over the k=1,...,K factors, so the sampler is
#' O(T*K*p^3) instead of O(T*(K*p)^3)
#'
#' @param Y the \code{T x m} data observation matrix, where \code{T} is the number of time points and \code{m} is the number of observation points (\code{NA}s allowed)
#' @param tau the \code{m x d} matrix of coordinates of observation points
#' @param X the \code{T x p} matrix of predictors; if NULL, only include an intercept
...
#' @import dsp KFAS truncdist
#' @export
dfosr_ar_k = function(Y, tau, X = NULL, K = NULL,
                      use_dynamic_reg = TRUE,
                      dist_reg_coef = "DHS", dist_reg_error = "NIG",
                      nsave = 1000, nburn = 1000, nskip = 3,
                      mcmc_params = list("beta", "fk", "alpha", "mu_k", "ar_phi"),
...
#' @import dsp KFAS truncdist
#' @export
dfosr_ind_k = function(Y, tau, X = NULL, K = NULL,
                       use_dynamic_reg = TRUE,
                       dist_reg_coef = "DHS", dist_reg_error = "NIG",
                       nsave = 1000, nburn = 1000, nskip = 3,
                       mcmc_params = list("beta", "fk", "alpha", "mu_k"),
                       add_p_shrink = FALSE,
                       use_obs_SV = FALSE,
                       includeBasisInnovation = FALSE,
                       computeDIC = TRUE){
...
#' @import dsp KFAS truncdist
#' @export
dfosr_rw_k = function(Y, tau, X = NULL, K = NULL,
                      use_dynamic_reg = TRUE,
                      dist_reg_coef = "DHS", dist_reg_error = "NIG",
                      nsave = 1000, nburn = 1000, nskip = 3,
                      mcmc_params = list("beta", "fk", "alpha"),
